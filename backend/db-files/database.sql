...
BEGIN
    ...
    WHERE course_id = @courseID
       OR prerequisite_course_id = @courseID;
...
exec Procedures_ViewOptionalCourse 5,'S23'
...
create FUNCTION [ExtraCourseRequests]
(@advisor_id int)
 RETURNs Table
AS
RETURN(SELECT R.* FROM Request R inner join Advisor A 
...
WHERE R.type = 'course')
go
select * from dbo.[ExtraCourseRequests] (2)
go

create FUNCTION [ExtraCHRequests]
(@advisor_id int)
 RETURNs Table
AS
RETURN(SELECT R.* FROM Request R inner join Advisor A 
on R.advisor_id = A.advisor_id and A.advisor_id = @advisor_id
WHERE R.type = 'credit_hours')
go
 
select* from Payment
select * from Installment
drop proc [ExtraCHRequests]
drop function [ExtraCHRequests]
 select * from  dbo.[ExtraCHRequests] (5)
...
exec Procedures_AdvisorApproveRejectCHRequest 12,'W23'
...
select * from dbo.ExtraCHRequests(1)
  -------------------------------------------------------------
...
exec [Procedures_ViewOptionalCourse] 5,'W24'
------------------------------------------------------------------------------------------
...
go
create proc getStudentSemCode
@StudentID int
AS
select semester_code from
Student s inner join Student_Instructor_Course_take sict on s.student_id = sict.student_id
where s.student_id = @StudentID
go

go
create proc AdvisorViewHisStudents
@advisor_id int
As
SELECT * FROM Student 
...
exec AdvisorViewHisStudents 1

go
create proc getSemCode
@request_id int
AS
SELECT semester_code from Request  r inner join Course_Semester cs on cs.course_id = r.course_id
...
go
create FUNCTION [getLatestStudent]
()
returns table
as
return (select max(student_id) as maximum from Student)
go

go
create FUNCTION [getLatestAdvisor]
()
returns table
as
return (select max(advisor_id) as maximum from Advisor)
go

select* from dbo.getLatestAdvisor()
select* from dbo.getLatestStudent()
--//////////////////////////////////////////////////////////////////////////////////////////////
...
select * from Student_Instructor_Course_take --hna fi moshkela
select * from Semester
select * from PreqCourse_course
select * from Request
select * from Slot --hna fi moshkela
select * from Payment --hna fi moshkela
select * from Course
select * from Course_Semester
select * from Graduation_Plan
select * from Installment --hna fi moshkela
select * from Instructor --hna kan fi moshkela
...
declare @x int
EXEC Procedures_StudentRegistration 'marwa','sis','12345','Engineering','Mar@gmail.com','CS',5,@x output 
print @x